...
public class RR {
    ...
    public static void RR(ArrayList<Process> processes, int number_of_processes, Scanner scanner, boolean verbose_flag) throws FileNotFoundException {
        ...
        while(finished_processes.size() < number_of_processes){
            ...
            if(verbose_flag){
                ...
                for(Process p : processes){
                    if(p.state.equals("blocked")){
                        System.out.print(String.format("%-16s", p.state + " " + Integer.toString(p.io_burst)));
                    }
                    else if(p.state.equals("running")){
                        System.out.print(String.format("%-16s", p.state + " " + Integer.toString(Math.min(p.cpu_burst, p.quantum))));
                    }
                    else{
                        int burst = 0;
                        System.out.print(String.format("%-16s", p.state + " " + Integer.toString(Math.min(burst, p.quantum))));
                    }
            ...
            if(running_process == null && !ready_processes.isEmpty()){
                ...
                if(ready_processes.peek().arrival_time != cycle || cycle == 0){
                    ...
                    //give the running process a quantum of 2
                    running_process.quantum = 2;
                }
